...
def get_num_attempts(app):
    while True:
        app.addLabelEntry('How many incorrect attempts')
        num_attempts = app.getEntry('How many incorrect attempts')
        try:
...
def play_hangman_gui(app):
    app.infoBox('', 'Starting a game of hangman')
    attempts_remaining = get_num_attempts(app)
    min_word_length = get_min_word_length()
